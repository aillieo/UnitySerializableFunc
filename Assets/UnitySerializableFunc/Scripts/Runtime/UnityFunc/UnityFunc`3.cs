// -----------------------------------------------------------------------
// <copyright file="UnityFunc`3.cs" company="AillieoTech">
...
namespace AillieoUtils.SerializableFunc
{
    ...
    [Serializable]
    public class UnityFunc<T0, T1, TResult> : UnityFuncBase
    {
        ...
        public void AddListener(System.Func<T0, T1, TResult> call)
        {
            this.AddCall(GetDelegate(call));
        }

        public void RemoveListener(System.Func<T0, T1, TResult> call)
        {
        ...
        public TResult Invoke(T0 arg0, T1 arg1)
        {
            ...
            {
                if (calls[i] is InvokableCall<T0, T1, TResult> curCall)
                {
                    result = curCall.Invoke(arg0, arg1);
                }
                else
                {
        ...
        {
            return new InvokableCall<T0, T1, TResult>(target, theFunction);
        }

        internal void AddPersistentListener(System.Func<T0, T1, TResult> call)
        {
            this.AddPersistentListener(call, UnityEventCallState.RuntimeOnly);
        }

        internal void AddPersistentListener(System.Func<T0, T1, TResult> call, UnityEventCallState callState)
        {
        ...
        internal void RegisterPersistentListener(int index, System.Func<T0, T1, TResult> call)
        {
            if (call == null)
            {
        ...
        private static BaseInvokableCall GetDelegate(System.Func<T0, T1, TResult> action)
        {
            return new InvokableCall<T0, T1, TResult>(action);
        }
    }
}
